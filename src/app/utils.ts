...
export const shortString = (numOfChars: number, text: string): string => {
  let toShort = 1;

  if (text.length <= 2) return text;

  if (numOfChars >= 1 && (text.length / 2 - 1) >= numOfChars) {
    toShort = numOfChars
  } else {
    console.log('Fuera del rango!!');
    
  }
  return text.slice(0, toShort) + '...' + text.slice(-toShort);
}
...
/**
 * ## Gets the user encrypted name
 * @param name User name to encrypt
 * @returns encrypted name
 */
